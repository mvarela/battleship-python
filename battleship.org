...
** Data Model & Implementation
   - The core of the problem lies in representing the board's state, and its transitions.
     - The grid itself can be represented as an list of lists, and within each
       cell in the grid, we represent the known state, which can be one of the
       following:
...
*** Setup
    ...
    We need some boilerplate to setup our project:
...
#+end_src
  ...
  Coming to our board, we can now create the basic functionality. We need to:
    [X] Create a new board, initializing the cells to =EmptyPiece= instances.
    [X] Check whether we can add a ship, given its orientation and a starting pair of =(x,y)= coordinates
    [X] Add a ship, given a pair of =(x,y)= coordinates, ship length, and an orientation (provided it can be added)
    [X] Fire at a location
    [X] Check whether the board is "lost" (that is, all ships have been sunk)
...
*** The Game
    With the board mechanics and AI in place, we can now create a =Game= class
    that will handle the playing and the necessary rules (e.g., board size,
    ships to be added, etc.). It may be helpful to also have a =Player= class to
    handle the player's turns, and also the player's board setup. Once we have
    the =Player= and =Game= ready we can code a CLI driver for actually playing
    the game.
    
   
#+begin_src python :exports code :results silent :eval never :noweb yes :tangle "battleship/player.py"
  class Player:
...
The =Game= itself contains both an =AI= and a =Player= instance, and alternates
turns playing them. For the =AI= interaction, it suffices to call into
=playTurn=. For the actual player, however, we need some interaction mechanism.
...
#+begin_src python :exports code :results silent :eval never :noweb yes :tangle "battleship/game.py"
  from battleship.board import Board
  from battleship.ai import AI
...
We can quickly test that a =Game= instance will behave correctly by driving a
dummy player who plays random positions.


#+begin_src python :exports code :results silent :eval never :noweb yes :tangle "battleship/test_game.py"
  from hypothesis import given, strategies as st
...
We exercise the =Game= API, and verify that every game ends, and ends with a
single winner.
...
#+begin_src python :exports code :results silent :eval never :noweb yes :tangle "battleship/cli.py"
    import sys, os
    from battleship.game import Game, Turn
